...
Every achievement-granted title a character has earned, shown as a small
italic line directly under their name at the top of the expanded panel —
not a fourth module like Equipped/Collections/PvP, just a subtitle, since
a title isn't "earned" separately from the achievement that grants it (no
date of its own, so no place in the Sort by: Date view either).

- **Data source**: no new characters.json field and no new DB query —
  titles are resolved entirely client-side by cross-referencing the
  character's existing `achievements` array against a bundled
  `assets/data/titles.json` (achievement id → title text), generated by
  `scripts/generate-titles-data.py` from the same `Achievement_3.3.5_12340.csv`
  `achievements.json` already comes from (its `Reward_lang[0]` field is
  Blizzard's own tooltip text for what an achievement grants, e.g. `"Title
  Reward: Elder"`).
- **Why not CharTitles.dbc**: titles are actually granted server-side via
  the `achievement_reward` world-DB table (a real CharTitles id per
  achievement, looked up live when it completes) — but unlike Item/
  Achievement/Talent data, nobody has published a CharTitles.dbc-derived
  CSV for this build from the same trusted source, and extracting one
  would need the raw client DBC file, which isn't available here. Reading
  the achievement reward tooltip text sidesteps that entirely.
- **Known gap, stated plainly**: this only covers achievement-granted
  titles. A handful of WotLK titles come from quests
  (`quest_template.RewardTitle`) instead and won't show up here — the
  large majority of titles are achievement-granted, but this isn't
  literally every title source the game has. Same kind of deliberate,
  documented gap as Collections' Blood Parrot exception, not an
  oversight — see the generator script's own docstring for the full
  reasoning and the couple of hand-verified text-cleanup rules it applies
  (a faction-conditional title, two fragments that need a word prepended
  to read correctly) on top of the straight tooltip-prefix strip.
...
[Titles](#titles) reads this same `achievements` array — there's no
separate `titles` field in this shape at all.
...
- **Titles** — every achievement-granted title, shown as a subtitle line
  under the character's name. Resolved entirely client-side from data
  already fetched (no new characters.json field, no new DB query) against
  a bundled achievement id → title text map. Deliberately achievement-only
  for now, not every title source in the game (see
  [Titles](#titles) above for the known gap).
