...
Each character row also shows a dim line under the name/level, the zone
they last logged out in — the same thing the real character-select
screen shows. `characters.zone` (a numeric `AreaTable.dbc` id, confirmed
against AzerothCore's own schema) is resolved client-side against
`assets/data/area_names.json` (id → English name, all 2307 entries),
generated by `scripts/extract-dbc-names.py` run against the actual
`AreaTable.dbc` file a real worldserver needs to boot — that binary
format itself isn't something AzerothCore's own repos or any community
mirror host as text, but the server operator's own copy of it is a real
primary source once extracted. Fetched separately from (and just as
eagerly as) the rest of this app's reference data, since it's needed on
the always-visible collapsed row rather than only inside the expandable
panel — a character with no `zone_id` (a stale `characters.json`) or an
...
The last module, right after Quests — a plain list of faction names, no
sub-values. A live snapshot like Equipped/Avg Item Lvl, not a
Collection: reputation standing keeps moving, so there's no `earned_at`
moment to record and no place in the Sort by Type/Date toggle.

- **Data source**: `character_reputation` (guid, faction, standing) —
  but `standing` turned out **not** to be the character's total
  reputation with a faction, it's a delta on top of a race/class-specific
  baseline. Confirmed against AzerothCore's own
  `ReputationMgr::GetReputation()`, which returns
  `GetBaseReputation(faction) + standing`, and `GetBaseReputation()`
  walks `Faction.dbc`'s 4 `(raceMask, classMask, baseValue)` slots for a
  match against the character's own race/class (bitmask
  `1 << (id - 1)`, confirmed against `Unit::getRaceMask()`/
  `getClassMask()`). Getting Exalted status right needed this real
  baseline data, not a guess — extracted by
  `scripts/extract-faction-baselines.py` from the operator's own
  `Faction.dbc` (the same file the worldserver needs to boot), bundled
  at `assets/data/faction_baselines.json` and used server-side in the
  export scripts (63 of `Faction.dbc`'s ~400 rows have a nonzero
  baseline for any race/class; everything else safely defaults to 0).
- **Exalted threshold**: `standing >= 42000`, universal across every
  faction — confirmed straight from AzerothCore's own
  `ReputationMgr::ReputationToRank()`, not assumed from general WoW
  knowledge.
- **Which factions count**: `Faction.dbc` has ~400 rows total, most of
  them NPC hostility factions or other non-trackable entries, not real
  "shows up on your reputation pane" factions — filtered to the ~105
  with `reputationListID >= 0` (`FactionEntry::CanHaveReputation()`).
  Which factions a character is currently Exalted with (a plain array of
  faction ids) is computed entirely server-side, in the export scripts.
  Names for just those ~105 trackable factions are bundled at
  `assets/data/faction_names.json` (id → English name); resolving an id
  to a name is the only part that happens client-side, the same way
  Titles resolves an achievement id to a title string — from a bundled
  reference file, not a second DB round-trip.
- **Blank fallback**: an empty or missing `exalted_factions` array hides
  this module entirely, same graceful-degradation pattern as every other
  module.
...
them. A thin divider (no header) separates it from whatever precedes
it, and (since it's the last thing in the grid, with no heading of its
own to force a new row the way every module's `.achv-section__name`
does) explicitly spans the full grid width so it can't end up placed
beside another module's content instead of below everything. Its date
...
everywhere else in this app, which is the wrong implication for a plain
current fact. No icon, unlike every other `.achv-list__item` — there's
no single established icon for "last logged in" the way Played
Time/Honor Points each have one. Blank (not shown at all) for a
...
see [PvP](#pvp) above. `last_online` is `characters.logout_time`
converted through `iso()` — see [Last logged in](#last-logged-in) above.
...
`zone_id` is `characters.zone`, read straight through; resolved to a
name client-side against `assets/data/area_names.json` — see the
[Character Dashboard](#character-dashboard) intro above.

`exalted_factions` is a plain array of `Faction.dbc` ids, computed
server-side against `assets/data/faction_baselines.json` and each
character's own race/class; resolved to names client-side against
`assets/data/faction_names.json` — see
[Exalted Factions](#exalted-factions) above.
...
[Quests](#quests) needs no separate query — a `LEFT JOIN` subquery
against `character_queststatus_rewarded` (`WHERE active = 1`, `GROUP BY
guid`) on the same main character query.

[Zone last logged in](#character-dashboard) needs no separate query
either — `characters.zone`, one more column on the main character query,
resolved to a name client-side.

[Exalted Factions](#exalted-factions) does need its own query, same
relationship shape as Talents (`character_reputation` isn't 1:1 with a
character — one row per faction the character has ever gained rep
with). The extra piece it needs beyond that query is each character's
own `race`/`cls` (already on the main query), to correctly compute the
per-race/class baseline `assets/data/faction_baselines.json` carries.
...
- **Last logged in** — moved out of PvP to the very last thing in the
  whole panel, after every other module (the character's own request) —
  see [Last logged in](#last-logged-in) above.
...
- **Zone last logged in** — a dim line under the name/level on each
  character row, resolved client-side against a real `AreaTable.dbc`
  extraction (`scripts/extract-dbc-names.py`, run against the
  operator's own worldserver data) — see the
  [Character Dashboard](#character-dashboard) intro above.
- **Exalted Factions** — the last module, right after Quests: a plain
  list of faction names. `character_reputation.standing` turned out to
  be a delta on a race/class-specific baseline, not the total
  reputation — confirmed against AzerothCore's own
