...
- **Data source**: `character_talent` (guid, spell, specMask) — a new
  temp-file query, same pattern as Achievements/Equipped Gear/Known
  Spells. Only the highest rank of a given talent is ever a row here,
  confirmed directly against `Player::addTalent`'s own "remove old
  talent rank if any" behavior in AzerothCore's own
  `PlayerStorage.cpp`/`Player.cpp` — so summing points per tree never
  needs to guard against double-counting a lower rank alongside its
  upgrade.
- **Dual spec**: WotLK has two talent specs per character, and
  `character_talent` keeps rows for both — `specMask` (bit 0 = spec 0,
  bit 1 = spec 1) marks which spec each row belongs to. Only the
  character's **currently active** spec is summed
  (`characters.activeTalentGroup`, added to the main character query),
  using the same bitmask AzerothCore's own `GetActiveSpecMask()` does —
  otherwise a respec'd character's two builds would mix together into
  one meaningless number.
- **Points per tree, computed server-side**: `assets/data/
  talent_spells.json` (a static `{spell id: {tab_id, points}}` map,
  generated by `scripts/generate-talent-data.py` from the real
  `Talent_3.3.5_12340.csv`/`TalentTab_3.3.5_12340.csv` DBC data, same
  `r-o-b-o-t-o/azerothcore-armory` source used elsewhere) turns each raw
  talent spell id into which tree it belongs to and how many points that
  rank represents — the export scripts sum these server-side into a
  compact `{tab_id: points}` per character, the same way
  `achievement_points` arrives as one precomputed number rather than a
  raw list of achievement ids.
- **Tree names/icons**: resolved client-side, `TALENT_TABS_BY_CLASS` in
...
[Talents](#talents) does need its own query — `character_talent` isn't
1:1 with a character the way `character_stats` is (multiple talents,
and up to two specs' worth of rows per character), same shape of
relationship Achievements/Equipped Gear/Known Spells already have. The
extra piece Talents needs beyond that query is
`characters.activeTalentGroup`, one more column on the main character
query, to know which of the two specs' rows to actually sum.
...
| Zone last logged out in (character-select-style, under the level/name in each character row) | Blocked | `characters.zone` (confirmed against AzerothCore's real schema) is a numeric `AreaTable.dbc` id, not a name — resolving it to text needs a zone-id-to-name reference, and that data is Blizzard client DBC content, not something AzerothCore's own repos or the community CSV mirror already used for Achievements/Talents host as text. General web lookups (wowhead, wowdev.wiki) are blocked by this session's network policy, so this is parked until a reachable primary source for zone names turns up |
| Exalted factions list | Blocked, same reason as Zone | `character_reputation` (guid, faction, standing, flags) is real and accessible, and the Exalted threshold (`standing >= 42000`, universal across every faction) is confirmed straight from `ReputationMgr::ReputationToRank` in AzerothCore's own source — the mechanics are solid. What's missing is `faction` id → name: not in the community CSV mirror, not in any of AzerothCore's 218 org repos, and GitHub code search (to check for named faction constants in the C++ source) now requires a login this session doesn't have. Same "reachable primary source not found yet" block as Zone, not a design problem |
